...
def calculate_cash_flow():
    try:
        data = request.json
        ticker = data.get('ticker')
        fcf_method = data.get('fcf_method', 'conservative')
...
@app.route('/api/earnings-growth', methods=['POST'])
def calculate_earnings_growth():
    try:
        ...
        if financials.empty:
            return jsonify({'error': f'No financial data found for {ticker}'}), 404

        # Initialize results dictionary and data list
        earnings_data = {}
        data = []
...
def calculate_roe():
    try:
        ...
        if financials.empty or balance_sheet.empty:
            return jsonify({'error': f'No financial or balance sheet data available for {ticker}'}), 404
...
def generate_roe_csv():
    try:
        ...
        if financials.empty or balance_sheet.empty:
            print(f"No financial or balance sheet data available for {ticker}")
            return None, None
